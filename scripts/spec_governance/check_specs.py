...
def find_registry_file(repo_root: Path) -> Path | None:
    candidates = [
        repo_root / ".agents" / "registry" / "agents.v1.yml",
...
def main():
    ...
    if not registry_file:
        print("ERROR: Could not find agent registry file. Expected .agents/registry/agents.v1.yml or .agents/registry/registry.yml", file=sys.stderr)
        return 2
